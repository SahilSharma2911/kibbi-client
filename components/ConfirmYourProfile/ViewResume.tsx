...
const ViewResume = () => {
    ...
    return (
        <div className="object-contain w-full border-8 rounded-md border-[#D9D9D9] px-6 pt-6 pb-10 min-h-[45rem] font-poppins ">

            {/* Name  */}
            <div>
                <h3 className='font-Poppins text-[2rem] font-extrabold uppercase'>
                    {resumeData?.["First Name"] || ""} {resumeData?.["Last Name"] || ""}
                </h3>
            </div>

            <article className='flex gap-6 mt-3'>
                {/* left side  */}
                <div className=' flex flex-col gap-4 w-[40%]'>
                    {/* contact info */}
                    {resumeData?.["Contact Information"] && (
                        <div>
                            <h3 className="text-[0.8rem] text-[#a28b91] border-[#AC9196] border-b-2 pb-1.5 pr-3 w-fit font-bold">
                                My Contact
                            </h3>
                            <ul className='mt-2 text-[0.6rem]'>
                            ...
                            <div className="">
                                {resumeData?.Education?.map((edu, index) => (
                                    <div key={index} className="flex gap-2 mt-2">
                                        ...
                                        <div className="flex flex-col text-[0.6rem] gap-0.5">
                                            {/* Render instituteName only when it exists */}
                                            {edu?.instituteName && (
                                                <span className="text-[0.65rem]">{edu.instituteName}</span>
                                            )}
                ...
                <div className=' w-[60%]'>
                    ...
                    {resumeData?.summary && (
                        <div>
                            ...
                            <p className='mt-2 text-[0.6rem] w-9/12'>
                                {resumeData.summary}
                            </p>
                        </div>
                    )}
                                ...
                                {resumeData.Certifications?.map((cert, index) => (
                                    <div key={index} className="flex gap-2 mt-1">
                                        ...
                                        <div className="flex flex-col text-[0.6rem]">
                                            ...
                                            {cert?.documentName && (
                                                <span className="text-[0.65rem]">{cert.documentName}</span>
                                            )}
